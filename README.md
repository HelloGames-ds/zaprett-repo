...
1. Сначала нужно [создать форк](https://github.com/CherretGit/zaprett-repo/fork) данного репозитория. После форка необходимо зайти в Actions и включить их, иначе все последующие шаги будут выполнены зря.
2. Затем создайте новый branch и внесите изменения — в новый или существующий файл. Название файла должно начинаться с `list-`, `ipset-`или `strategy-`.  
...
4. Затем откройте файл `hosts.json`, `strategies.json` или `ipsets.json` (в зависимости от того, что вы редактировали), найдите ваш файл и отредактируйте следующие поля:
   ```json
   {
